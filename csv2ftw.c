...
// frequency tuning word (FTW) data (4 bytes per FTW)
	...
	while(1)
	{
		...
		if ((ret!=1) || (c==',') || (c=='\n') || c=='\r' || (c==' '))
		{
			...
			else
			{
				...
				if (ctr%3==0) // do a newline every so often
				{
					fprintf(outfile, "\n");
				}
				ctr++;
				fprintf(outfile, "0x%02x, 0x%02x, 0x%02x, 0x%02x", data[0], data[1], data[3], data[4]);
			}
		}
		else
		{
